...
struct Buffer {
    ...
    Buffer(VmaAllocator allocator, const VkBufferCreateInfo& bufferInfo, const VmaAllocationCreateInfo& allocInfo);
...
int main(int argc, char** argv) {
    ...
    auto inputBuffer = Buffer(ctx.allocator, bufferCI, bufferAllocInfo);
    
    {
        float *pData = nullptr;
    
        inputBuffer.map(reinterpret_cast<void **> (&pData));
        std::copy(inputData.begin(), inputData.end(), pData);
        inputBuffer.unmap();
    }

    bufferAllocInfo.usage = VMA_MEMORY_USAGE_GPU_TO_CPU;
    auto outputBuffer = Buffer(ctx.allocator, bufferCI, bufferAllocInfo);
    ...
    std::cout << "Output: [";
    {
        ...
        outputBuffer.map(reinterpret_cast<void **> (&pResults));
...
context::context() {
    ...
    vkFn.vkAllocateMemory = vkAllocateMemory;
    vkFn.vkBindBufferMemory = vkBindBufferMemory;
    vkFn.vkBindImageMemory = vkBindImageMemory;
    vkFn.vkCreateBuffer = vkCreateBuffer;
    vkFn.vkCreateImage = vkCreateImage;
    vkFn.vkDestroyBuffer = vkDestroyBuffer;
    vkFn.vkDestroyImage = vkDestroyImage;
    vkFn.vkFlushMappedMemoryRanges = vkFlushMappedMemoryRanges;
    vkFn.vkFreeMemory = vkFreeMemory;
    vkFn.vkGetBufferMemoryRequirements = vkGetBufferMemoryRequirements;
    vkFn.vkGetBufferMemoryRequirements2KHR = vkGetBufferMemoryRequirements2KHR;
    vkFn.vkGetImageMemoryRequirements = vkGetImageMemoryRequirements;
    vkFn.vkGetImageMemoryRequirements2KHR = vkGetImageMemoryRequirements2KHR;
    vkFn.vkGetPhysicalDeviceMemoryProperties = vkGetPhysicalDeviceMemoryProperties;
    vkFn.vkGetPhysicalDeviceProperties = vkGetPhysicalDeviceProperties;
    vkFn.vkInvalidateMappedMemoryRanges = vkInvalidateMappedMemoryRanges;
    vkFn.vkMapMemory = vkMapMemory;
    vkFn.vkUnmapMemory = vkUnmapMemory;

    VmaAllocatorCreateInfo allocatorInfo {};
    allocatorInfo.physicalDevice = physicalDevice;
    allocatorInfo.device = device;
    allocatorInfo.pVulkanFunctions = &vkFn;
...
Buffer::Buffer(VmaAllocator allocator, const VkBufferCreateInfo& bufferInfo, const VmaAllocationCreateInfo& allocInfo) {
    this->allocator = allocator;

    vmaCreateBuffer(allocator, &bufferInfo, &allocInfo, &handle, &memory, nullptr);
}
...
void Buffer::map(void ** ppData) {
    vmaMapMemory(allocator, memory, ppData);
}
